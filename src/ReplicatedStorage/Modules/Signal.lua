...
function Signal:Connect(handler)
	...
	return self._bindableEvent.Event:Connect(function(queueId)
		coroutine.wrap(function()
			...
			repeat task.wait(); timeout += 1; until timeout > 50 or self._cooldown == false;
			if timeout > 50 then
				error("timed out event because of cooldown")
			end
			timeout = 0;
			repeat task.wait(); timeout += 1; until timeout > 50 or self._queue[queueId];
			if timeout > 50 then
				error("timed out event because no queue data exists")
			end
